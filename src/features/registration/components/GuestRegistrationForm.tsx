  ...
  const handleGuestRegistration = async (data: RegistrationFormValues) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        // Display different success message based on payment type
        if (data.paymentType === "CASH") {
          toast.success(
            "Registration successful! You can ignore the QR code since you chose Cash payment.",
          );
        } else {
          toast.success("Registration successful!");
        }

        // Call the success callback with payment type
        onRegistrationSuccess(
          {
            firstName: data.firstName,
            lastName: data.lastName || "",
      ...
      <FormProvider {...methods}>
        ...
        >
          ...
          <div className="mb-6">
            ...
            <PaymentMethodSelector register={register} setValue={setValue} />
          </div>
